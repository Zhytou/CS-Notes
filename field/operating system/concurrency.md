...
进程控制块（Process Control Block，PCB）是操作系统为每个进程维护在内核中的一个数据结构，保存着进程的相关信息。在Linux中，它通常由一个名为`task_struct`的结构体表示。
...
struct task_struct {
  ...
  // 2. 进程状态
  volatile long state;         // 运行态/睡眠态/僵尸态等

  // 3. 内存管理（关联 mm_struct）
  struct mm_struct *mm;        // 进程地址空间管理结构
  struct mm_struct *active_mm;

  // 4. 上下文信息（寄存器/栈）
  struct thread_struct thread; // 存储 CPU 寄存器、栈指针等
  ...
  // 7. 其他核心字段（信号、资源限制、命名空间等）
  struct signal_struct *signal;
  struct sighand_struct *sighand;
  struct nsproxy *nsproxy;     // 命名空间（PID/网络/UTS等）
  // ... 还有上百个字段，此处简化
};
```
...
一个进程的虚拟地址空间承载了程序运行所需的全部状态。在32位架构下，受限于寻址位数，进程的理论地址空间上限为4GB（即$2^{32}$字节），如图3所示。而64位进程的理论寻址空间则呈爆炸式增长，达到 $2^{64}$ 字节（约16EB），远超当前任何物理内存的硬件极限。不过，考虑到现代CPU硬件、操作系统设计以及成本的限制，目前的x86_64处理器通常只实现了48位的虚拟地址寻址，也即对应着256TB的进程空间。

![图4 进程地址空间](../img/process_address.png)

如图4所示，进程空间大体上可以分成两大部分，即用户空间和内核空间。其中，前者也就是实现进程隔离性的核心。而后者虽然是共享的（映射了同一段物理内存），但仅仅只有当进程需要执行系统调用，并切换到内核态之后才会真正访问。（有关内核空间这部分的相关知识会在进程调度章节中更详细说明。）
...
在Linux系统中，进程虚拟地址空间的所有地址信息均存储在进程的`mm_struct`结构体中。其中，`start_xxx`和`end_xxx`字段分别表示该段内存的起始虚拟地址和结束虚拟地址。`vm_area_struct`结构体则用于记录进程地址空间中的每个内存区域（如代码段、数据段、栈段等）的详细信息，包括起始虚拟地址、结束虚拟地址、权限标志（如可读、可写、可执行等）等，进而帮助操作系统快速判断“某虚拟地址是否合法”“该地址有什么权限”。

```c++
struct mm_struct {
  // 1. 进程地址空间的核心标识
  unsigned long start_code, end_code;    // text段：起始/结束虚拟地址
  unsigned long start_data, end_data;    // data段：起始/结束虚拟地址
  unsigned long start_brk, brk;          // 堆区：起始/当前堆顶虚拟地址
  unsigned long start_stack;             // 主线程栈：起始虚拟地址（高地址）
  unsigned long start_bss, end_bss;      // bss段：起始/结束虚拟地址（部分内核版本合并到data相关字段）
  
  // 2. 内存区域链表（最核心！记录所有内存段的详细映射）
  struct vm_area_struct *mmap;           // 指向所有内存区域的链表头
  struct rb_root mm_rb;                  // 内存区域的红黑树（加速查找）
  
  // 3. 页表相关（虚拟地址→物理地址的映射表）
  pgd_t *pgd;                            // 页全局目录（页表的顶级指针）
  
  // 4. 权限与大小限制
  unsigned long total_vm;                // 进程使用的总虚拟内存大小
  unsigned long locked_vm;               // 锁定的内存页（不被换出）
  unsigned long flags;                   // 内存标志（如MM_WRITEABLE、MM_EXECUTABLE）
  
  // 5. 共享与计数
  atomic_t mm_users;                     // 使用该mm_struct的线程数（多线程共享时计数）
  atomic_t mm_count;                     // 引用计数（销毁时判断）
  struct task_struct *owner;             // 所属进程（主线程）
};
```
...
具体到Linux中，实际就是将当前上下文保存到进程的`thread_struct`结构体中，而将即将执行的进程的上下文恢复到CPU寄存器中。
...
![图 5 session & terminal & process group](https://jyywiki.cn/pages/OS/img/tty-session.png)
...
线程控制块（Thread Control Block，TCB）是操作系统为每个线程维护在内核中的一个数据结构，保存着线程的相关信息。在Linux系统中，线程本质是“轻量级进程”，因此内核并未单独定义 TCB结构体，而是直接复用`task_struct`来表征线程。
...
struct task_struct {
  ...
  // 3. 内存管理（关联 mm_struct）
  struct mm_struct *mm;        // 线程地址空间管理结构
  struct mm_struct *active_mm;
...
因此，当新线程创建时，内核便会基于父线程的`task_struct`创建一个全新的实例，并针对性更新关键字段，包括：
...
struct thread_struct {
  // 1. 通用寄存器（存储函数执行的临时数据）
  unsigned long rax, rbx, rcx, rdx; // 通用寄存器
  unsigned long rsi, rdi, rbp, rsp; // 栈基指针(rbp)、栈顶指针(rsp)
  unsigned long r8, r9, r10, r11;   // 扩展通用寄存器
  unsigned long r12, r13, r14, r15;

  // 2. 指令指针（核心！记录函数执行到哪一行）
  unsigned long rip; // 程序计数器(PC)，指向当前要执行的指令地址（比如函数的某一行机器码）

  // 3. 段寄存器（关联线程栈/TLS）
  unsigned short es, ds, fs, gs; // fs/gs 段寄存器指向当前线程的 TLS 区域
  unsigned long fs_base, gs_base; // TLS 区域的基地址

  // 4. 状态寄存器（记录 CPU 状态，如是否开启中断、运算标志）
  unsigned long eflags;

  // 5. 栈相关（线程栈的核心标识）
  unsigned long kernel_stack; // 内核栈地址（线程在内核态的栈）
  struct vm_area_struct *stack_vma; // 用户态线程栈的虚拟内存区域描述

  // 6. 其他上下文（浮点寄存器、信号处理、调试相关）
  struct fpstate *fpstate; // 浮点/向量寄存器（执行浮点运算时用）
  unsigned long sigmask;   // 线程的信号掩码
};
```
