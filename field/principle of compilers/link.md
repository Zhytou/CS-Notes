...
正如上一节中所提到那样，编译生成的ELF文件会得到一个符号表，即.symtab节。它会存放该ELF文件中定义和引用的函数和全局变量的信息。

因为多文件程序往往会出现一个符号在A文件中使用，但定义在B文件中的情况，所以链接器会先进行符号解析。即遍历所有目标文件中的符号引用，并在合并后的符号表中查找对应的定义。如果能够找到匹配的定义，就将引用与定义进行绑定。如果找不到匹配的定义，就会报链接错误。

### 重定位

此外，由于链接器会将多个目标文件合并（名字相同的section合并得到segment），所以需要更新合并后符号表中的地址偏移量。这就被称作重定位。
