...
- 当DHCP服务器收到DHCP发现报文时，用DHCP提供报文(DHCP OFFER)向客户端做出响应。该报文仍然使用广播地址255.255.255.255作为目标地址，因为此时DHCP客户端并没有一个IP地址。此外，该数据报中信息包括：服务器提供可租约的IP地址、子网掩码、默认网关、DNS服务器以及IP地址租用时间。
...
假设有两个客户端192.168.1.10和192.168.1.11同时与服务器183.232.231.172进行通信，并且这两个客户端的本地端口都是1025。使用NAT技术进行转换，将这两个私有IP地址都转换为公有地址120.229.175.121，但是以不同的端口号作为区分。换句话说，NAT重用公网IP以端口号进行区分（即网络地址与端口转换技术，NAPT），并将转换关系保存在映射表中以保证TCP之类的持续通信。
...
路由器的核心功能是根据路由控制表中的记录，将IP数据包发送到正确的下一跳。其中的路由控制表记录了目标网络地址及与之对应的下一跳路由器的地址。在发送IP数据包时，路由器首先检查数据包首部中的目标IP地址，然后在路由控制表中查找与之具有相同网络地址的记录项。如果存在多条匹配记录，则选择最长前缀匹配原则，即网络地址前缀最长的那一条记录。根据该记录的下一跳地址，将数据包转发至相应的下一个路由器。正因为如此，我们称路由器工作在网络层。相比之下，交换机工作在较低的数据链路层，只根据MAC地址对数据帧进行转发，缺乏路由决策能力。

除了核心的路由转发功能，路由器内部还设有数据包缓冲区。当路由器正在处理某个数据包时，后续到达的数据包将先存入缓冲区排队等候处理。只有当缓冲区已满，无法再存储时，路由器才会丢弃新到达的数据包。此外，现代路由器还集成了许多其他功能。比如，内置了调制解调的功能，代替拨号上网中的调制解调器；提供NAT网关，帮助私有IP转换；充当DHCP服务器，避免每次启动手动配置主机IP等等。
