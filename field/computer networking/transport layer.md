...
接收方每接收到一个包就会向发送方发送确认信息，而发送方则通过接收方返回的确认号决定可发送的序号。
...
TCP之所以是一种可靠的协议，正是因为其自动重传的机制。自动重传请求（Automatic Repeat-reQuest, ARQ）是OSI模型中数据链路层和传输层的错误纠正协议之一。它通过使用确认和超时这两个机制，在不可靠服务的基础上实现可靠的信息传输。

具体来说，如果发送方在发送后一段时间之内没有收到确认信息，即ACK报文，它通常会重新发送，直到收到确认或者重试超过一定的次数。
...
- 快重传和快恢复（Fast Retrasmition and Fast Recovery）：
  - 快重传要求接收方每收到一个失序的报文段就立即发出确认报文，而不要等到自己发送数据时才捎带确认。同时，它会规定发送方在接收到3个重复确认时，直接重传丢失段，而不必继续等待为其设置的重传计时器到期。
...
TCP是一种面向字节流的协议,发送和接收数据时不考虑数据边界。
沾包指接收端一次接收了多个数据段的数据,形成一个较长的"数据包"。
拆包指接收端一次只能接收部分数据,形成较小"数据包",需要重新组装才能还原完整数据段。
TCP本身并没有处理沾包和拆包的机制,通常由应用层协议或socket API对传输数据流进行定界和处理。
