...
尽管宏定义常量和函数非常方便，但我们仍然跟偏向于使用一些C++提供的更安全的机制来替代宏。

**内联函数**：

内联函数是通过inline关键字定义的函数，用于请求编译器将函数的定义内联展开。通过内联展开的方式，就避免函数参数压栈/寄存器、CPU执行流切换、返回值存储等资源消耗的问题。

当然，内联函数也通常是针对逻辑简单的小函数。因为这种函数的的调用切换时间往往大于其执行时间。因此，反复调用的小函数非常适合写作内联函数进行优化。
