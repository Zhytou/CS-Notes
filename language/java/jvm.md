...
- 线程共享：堆、方法区, 堆外内存（JDK的永久代或JDK8的元空间、代码缓存）
...
在 JVM 的运行时数据区中，每个栈帧（Stack Frame）内部都持有一个指向运行时常量池（Runtime Constant Pool）中该栈帧所属方法的引用。持有这一引用的核心目的，是为了支持方法调用过程中的动态连接。

与 C/C++ 等编译型语言不同，Java 的编译过程并不包含传统的“链接”阶段。在编译生成的 Class 文件中，所有的方法调用仅以**符号引用（Symbolic Reference）**的形式存在——即一组用于描述目标方法的符号（如方法名、参数类型、返回值），而非方法在实际内存布局中的入口地址。
...
准确来说，JNI可以被称为Java的ABI（Application Binary Interface）。它通过以下三个组件协同工作：

- Java 层：使用 native 关键字声明方法，并通过 System.loadLibrary 加载二进制库（.so 或 .dll）。
- JNI 接口层（jni.h）：这是一组由 JDK 提供的 C/C++ 头文件，定义了 Java 类型与 C 类型的映射关系（如 jstring, jint）。
- C/C++ 实现层：按照特定规范编写代码，编译成动态库。

```java
public class NativeLib {
  static { System.loadLibrary("my_jni_lib"); }
...
Java_NativeLib_add(JNIEnv *env, jobject obj, jint a, jint b) {
  return a + b; // 直接在 C++ 层面计算并返回
}
```
...
方法区(Method Area)与堆一样，是各个线程共享的内存区域。它用于存储已被虚拟机加载的类型信息、常量、静态变量、即时编译器编译后的代码缓存等数据。准确来说，方法区的存储内容包括：

- 类完整信息：涵盖类的元数据、字段定义、方法定义（含方法字节码、参数列表、访问修饰符等）；
- 每个类独立维护一个常量池：包括符号引用、字符串字面量、数值常量等；
- 静态变量：类级别的静态成员变量；
- 其他数据：JIT 编译后的机器码缓存、方法表等执行辅助数据。

**方法区 vs 永久代 vs 元空间**：

方法区只是JVM规范中定义的一个概念，用于存储类信息、常量池、静态变量、JIT编译后的代码等数据，并没有规定如何去实现它，不同的厂商有不同的实现。其中，永久代是JDK7之前的方法区实现，而元空间则是JDK8及以后的方法区实现。

二者的核心差异在于：

- 永久代：在物理上是堆的一部分，和新生代、老年代地址是连续的，且受垃圾回收器管理；
- 元空间：存储于堆外的本地内存（操作系统内存），不再占用 JVM 堆空间，默认不受 JVM 垃圾回收器直接管理。

**JDK1.8 方法区变化**：

具体来说，JDK8之后方法区实现从永久代到元空间的变化包括：
...
其中，哪些内存需要回收已经给出了明确定义，下面就介绍如何根据这个定义去具体找出这些需要回收的标记算法。它主要包括：引用计算法和可达性分析法两种。
...
最早出现也是最基础的垃圾收集算法是标记-清除算法，在1960年由Lisp之父John McCarthy所提出。如它的名字一样，该算法分为标记和清除两个阶段：它首先标记出所有需要回收的对象，标记完成后统一回收掉所有被标记的对象。
...
Garbage First收集器，简称G1收集器，是垃圾收集器技术发展历史上的里程碑式的成果。在设计之初，它就提出了在一个长度为M毫秒的时间片段内，消耗在垃圾收集上的时间大概率不超过N毫秒这样的目标。

为了实现这个目标，G1收集器开创了收集器面向局部收集的设计思路和基于Region的内存布局形式。具体来说，在G1收集器出现之前的所有其他收集器，包括CMS在内，其垃圾收集的目标范围都可以分成三种，包括：

- Minor GC：面向新生代的垃圾回收；
- Major GC：面向老年代的垃圾回收；
- Full GC：面向整个Java堆的垃圾回收。
