...
typename std::remove_reference<T>::type&& move(T&& t) {
    // 1. remove_reference 剥离 T 可能携带的引用，拿到纯粹的类型
    // 2. 强制转换成 该类型&&
...
HTTP 通用、跨平台，但冗余、慢，适合对外暴露接口→ 客户端 / 前端 / 第三方对接用 HTTP。
RPC 高效、二进制、长连接、内置微服务治理，适合内部高性能调用→ 微服务之间用 RPC。
...
Delete 是用于从数据库表中删除指定的行数据的操作。
Delete 语句执行后，会将符合条件的行数据从表中删除，但不会删除表本身。
Delete 操作是一种逐行操作，它会触发相关的触发器（Triggers）和约束（Constraints），并且可以通过 Where 子句指定删除的条件。
Delete 操作会慢慢地执行，因为它需要逐行删除数据并记录日志，可以回滚。

Drop 是用于删除数据库对象（如表、索引、视图、存储过程等）的操作。
Drop 操作会将整个数据库对象从数据库中永久删除，包括对象本身及其所有相关的数据和定义。
Drop 操作是一种快速的操作，因为它不需要逐行删除数据，但需要谨慎使用，因为删除后无法回滚。

Truncate 是用于快速清空表中所有数据的操作。
Truncate 操作会删除表中所有数据，但保留表结构和定义。
Truncate 操作是一种快速的操作，因为它不需要逐行删除数据，但它不会触发触发器和约束，也无法带有 Where 子句，且无法回滚。
...
76、递归实现的斐波那契数列的时间和空间复杂度分别是多少？

​时间复杂度为O(2ⁿ)。​这是因为每次调用 fib(n) 会分裂为两个子调用 fib(n-1) 和 fib(n-2)，形成二叉树结构。

而​空间复杂度则为O(n)。因为​递归采用深度优先策略，同一时间仅保留单一路径的调用栈，因此最大空间占用为线性增长。

77、脚本文件第一行的#!/bin/bash作用是什么？？操作系统如何区分脚本文件或者二进制可执行文件？

操作系统依靠魔数识别二进制文件格式。其中，魔数是指这些文件开头的一段固定字节，用来标识文件格式。比如：

- ELF（Linux 程序）开头：7F 45 4C 46→ .ELF
- PE（Windows exe/dll）开头：4D 5A→ MZ
- JPG开头：FF D8 FF
- PNG开头：89 50 4E 47 0D 0A 1A 0A→ 对应 .PNG
- GIF开头：47 49 46 38 39 61→ 对应 .GIF

相比之下，文本文件则没有固定的开头字节。不过，可执行脚本则会以`#!`开头，比如：`#!/bin/bash`。这行叫Shebang（释伴行），作用是告诉操作系统：执行这个脚本文件时，使用 /bin/bash 这个解释器来运行。
